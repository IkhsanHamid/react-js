...
const EditUser = (props) => {
  const { username, id, firstname, lastname } = props.data;
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const handleRegistration = async (data) => {
    // console.log(data);
    const update = await apiMethod.UpdatedUserandCust(data);
    setPesan("sukses");
  };
  ...
  useEffect(() => {
    let defaultValue = {}
    defaultValue.id = props.users.id
    defaultValue.user_name = props.users.user_name
    defaultValue.firstname = props.users.firstname
    defaultValue.lastname = props.users.lastname
    // reset
  }, []);
